...
function daysLeftLabel(validUntil: string, status: QuotationStatus) {
...
function probability(status: QuotationStatus) {
  switch (status) {
    case "Accepted":
    case "Invoiced":
...
function eSignBadge(status?: Quotation["signatureStatus"]) {
  if (status === "Signed") {
    return (
...
export function QuotationsPage() {
  ...
  useEffect(() => {
    ...
    setCreateOpen(true);
    router.replace("/finance/quotations", { scroll: false });
  }, [router]);
  ...
  const acceptedCount = data.filter(
    (e) => e.status === "Accepted" || e.status === "Invoiced",
  ).length;
  ...
  const eSignPending = data.filter((e) => e.signatureStatus === "Pending").length;
  const signedAccepted = data.filter(
    (e) =>
      e.signatureStatus === "Signed" ||
      e.status === "Accepted" ||
      e.status === "Invoiced",
  ).length;
  ...
  const activity = useMemo(() => {
    ...
    for (const item of data) {
      ...
      events.push({
        id: `${item.id}-created`,
        title: `New quotation ${item.quotationId} created`,
        sub: `By ${item.createdBy || item.owner || "team"} • ${relativeTime(item.createdAt)}`,
        tone: "new",
        at: created,
      });
      if (item.sentAt || item.signatureStatus === "Pending" || item.status === "Sent") {
        events.push({
          id: `${item.id}-sent`,
          title: `Quotation ${item.quotationId} sent for e-sign`,
          sub: `To ${item.clientName} • ${relativeTime(item.sentAt || item.createdAt)}`,
          tone: "sent",
          at: parseWhen(item.sentAt)?.getTime() ?? created,
        });
      }
      if (item.signatureStatus === "Signed" || item.status === "Accepted" || item.status === "Invoiced") {
        events.push({
          id: `${item.id}-accepted`,
          title: `Quotation ${item.quotationId} was signed & accepted`,
  ...
  return (
    <div className="min-h-full w-full bg-[#F4F7FB] p-4 sm:p-6 lg:p-8 text-slate-900">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-[26px] font-bold tracking-tight text-slate-900">Quotations</h1>
            <span
              className={cn(
          ...
          <p className="mt-1 text-[13px] text-slate-400">
            Manage your quotations, track progress and stay on top of your business.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#5B4BDB] via-[#6E5AE8] to-[#8B6CF6] p-5 text-white shadow-sm">
          ...
          <p className="relative mt-1 text-[11px] text-white/80">
            {pendingItems.length} Open quote pipeline value
          </p>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-100 bg-gradient-to-br from-white to-emerald-50/70 p-5 shadow-sm">
          <div className="flex items-start justify-between">
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Accepted Quotes</p>
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-100 text-emerald-600">
              <CheckCircle2 className="h-4 w-4" />
            </span>
          </div>
          <p className="mt-3 text-[26px] font-bold text-slate-900">{acceptedCount}</p>
          <p className="mt-1 text-[11px] font-semibold text-emerald-600">Ready to invoice</p>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-100 bg-gradient-to-br from-white to-sky-50/80 p-5 shadow-sm">
          <div className="flex items-start justify-between">
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Quotes</p>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-[11px] text-slate-400">
                {acceptedCount} of {totalCount} Quotes converted
              </p>
            </div>
                  ...
                  {paginatedData.length === 0 ? (
                    ...
                    paginatedData.map((item) => {
                      ...
                      return (
                        <tr
                          key={item.id}
                          onClick={() => router.push(`/finance/quotations/${item.id}`)}
                        ...
                        >
                          ...
                          <td className="px-4 py-4 whitespace-nowrap">{eSignBadge(item.signatureStatus)}</td>
                          <td className="px-4 py-4 whitespace-nowrap">
                                  ...
                                  <div className="absolute right-0 z-20 mt-1 w-44 rounded-xl border border-slate-200 bg-white py-1 text-left text-xs text-slate-700 shadow-lg">
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        router.push(`/finance/quotations/${item.id}`);
                                      }}
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
              ...
              <div className="space-y-4">
                {activity.length === 0 ? (
                  <p className="py-6 text-center text-[12px] text-slate-400">No recent quotation activity.</p>
                ) : (
                  activity.map((event) => (
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
              ...
              <div className="flex items-center gap-4">
                <div className="min-w-0 flex-1 space-y-2">
                  <FunnelBar label="Total Quotes" count={totalCount} width="100%" tint="bg-violet-100 text-violet-800" />
                  <FunnelBar label="Sent" count={sentCount} width="85%" tint="bg-blue-100 text-blue-800" />
                  <FunnelBar label="E-Sign Pending" count={eSignPending} width="65%" tint="bg-amber-100 text-amber-800" />
                  <FunnelBar
                    label="Signed & Accepted"
                    count={signedAccepted}
                    width="50%"
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <div className="space-y-2.5">
              <ActionRow
                ...
                hint="Start a new quotation from scratch"
                onClick={() => setCreateOpen(true)}
              />
              <ActionRow
                icon={FileCheck}
                ...
                hint="Convert accepted quotes to invoice"
              ...
              <ActionRow
                ...
                hint="Track client digital signatures"
                onClick={() => router.push("/signature/documents")}
              />
              <ActionRow
                icon={BarChart3}
                iconClass="bg-violet-100 text-violet-600"
                title="Reports & Analytics"
                hint="View detailed quote performance"
      ...
      <CreateQuotationForm
        ...
        onCreated={() => setData(listQuotations())}
