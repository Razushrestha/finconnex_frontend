...
import { Home, Plus, Search, Download, Banknote } from "lucide-react";
import {
  PAYMENT_STATUSES,
  payments as seed,
  listPayments,
...
export default function PaymentsPage() {
  ...
  function exportCsv() {
    const header = ["ID", "Invoice", "Client", "Amount", "Method", "Status", "Received"];
        ...
        <div className="mb-2.5 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              type="button"
              onClick={() =>
                router.push("/finance/payments/create?layoutid=standard&redirect=false")
              }
          ...
          <table className="w-full text-left text-[12px]">
            ...
            <tbody>
              {paginated.map((r) => (
                <tr
                  key={r.id}
                  onClick={() => router.push(`/finance/payments/${r.id}`)}
                  className="cursor-pointer border-t border-slate-50 hover:bg-violet-50/40"
                >
                  <td className="px-4 py-3">
                    <div className="font-semibold text-slate-900">{r.paymentId}</div>
                    <div className="text-[11px] text-slate-500">{r.receivedAt}</div>
                  </td>
                  <td className="px-3 py-3 text-slate-700">{r.invoiceRef}</td>
                  <td className="px-3 py-3 text-slate-600">{r.clientName}</td>
                  <td className="px-3 py-3 text-slate-600">{r.method}</td>
                  <td className="px-3 py-3">
                    <span
                      className={cn(
                        "rounded-full px-2 py-0.5 text-[9px] font-semibold",
                        PAYMENT_STATUS_STYLE[r.status],
                      )}
                    >
                      {r.status}
                    </span>
