...
export function ReportWorkspace({
  ...
  const rowNoun = reportDef.id.includes("source") ? "sources" : "rows";

  function clearFilters() {
    setFilters({
      ...defaultLibraryFilters(),
      dateRange: loadLibraryRange(),
      groupBy: reportDef.groupBy?.[0]?.id,
    });
    setPage(1);
  }
  ...
  return (
    <div className="min-h-full bg-[#F4F6F9]">
      <div className="mx-auto flex w-full max-w-[1920px] flex-col gap-4 p-4 lg:px-6 2xl:px-8 2xl:py-5">
        ...
        {tab === "summary" && viz === "chart" && result && "chart" in result && result.chart?.points.length ? (
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-4">
              ...
              <div className="mt-2 h-56">
                <ReportChart
                  ...
                  donutTotal={typeof result.kpis[0]?.value === "number" ? result.kpis[0].value : result.chart.points.reduce((n: number, p: { value: number }) => n + p.value, 0)}
                />
              </div>
            </section>
            {showFunnel ? (
