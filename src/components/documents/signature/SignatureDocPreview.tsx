...
export function SignatureDocPreview({
  ...
  // Check if fileName is an object URL, blob, or remote HTTP URL
  const isRealFile =
    fileUrl &&
  ...
  return (
    ...
    >
      {isRealFile ? (
        // Render the actual uploaded document inside an iframe or background layer
        <iframe
        ...
        // Fallback mock skeleton lines if only a name string is passed
        <>
          <div className="absolute inset-x-8 top-10 space-y-2">
            <div className="h-3 w-2/3 rounded bg-slate-200/80" />
            <div className="h-2 w-full rounded bg-slate-100" />
      ...
      {fields.map((f) => {
        ...
        return (
          <div
            key={f.id}
            className={cn(
              "absolute flex items-center justify-center gap-1 overflow-hidden rounded border-2 border-dashed px-1.5 py-1 text-[10px] font-semibold shadow-sm transition-shadow z-10",
              color.bg,
              color.text,
              color.border,
              dim,
